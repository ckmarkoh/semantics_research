...
class CwnInterface(SQLiteInterface):
    ...
    def get_lemma_relation(self,lemma):
        ...
        for relation in relation_ary_raw:
            if relation['rel_type'] not in relation_result.keys():
               relation_result.update({relation['rel_type']:[]})
            relation_result.get(relation['rel_type']).append(re.sub(r'[0-9]','',relation['rel_lemma']))
